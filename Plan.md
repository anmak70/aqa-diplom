...
### 2. Проверка покупки тура по дебетовой карте:
  #### 2.1 Проверка покупки тура по дебетовой карте с валидным заполнением всех полей
    ...
    Ожидаемый результат - запрос отправляется в банк,  банк присылает ответ - "Успешно. Операция одобрена Банком".
  ...
  #### 2.2 Проверка покупки тура по дебетовой карте с валидным заполнением всех полей кроме поля "номер карты"
    ...
    1. Заполнить поля: карта 4444 4444 4444  
                       ...
                       код CVC - 789
    2. Кликнуть кнопку "Продолжить".
    Ожидаемый результат - появление подсказки - "Неверный формат" -> запрос в банк не отправляется.
  ...
  #### 2.3 Проверка покупки тура по дебетовой карте с валидным заполнением всех полей кроме поля "месяц - 13"
    ...
    1. Заполнить поля: карта 4444 4444 4444 4441
                       ...
                       код CVC - 789
    2. Кликнуть кнопку "Продолжить".
    Ожидаемый результат - появление подсказки - "Неверно указан срок действия карты" -> запрос в банк не отправляется.  
  ...
  #### 2.5 Проверка покупки тура по дебетовой карте с валидным заполнением всех полей кроме поля "год"
    ...
    1. Заполнить поля: карта 4444 4444 4444 4441
                       ...
                       код CVC - 789
    2. Кликнуть кнопку "Продолжить".
    Ожидаемый результат - появление подсказки - "Истек срок действия карты" -> запрос в банк не отправляется.  
  ...
  #### 2.6 Проверка покупки тура по дебетовой карте с валидным заполнением всех полей кроме поля "Владелец - цифрами"
    ...
    1. Заполнить поля: карта 4444 4444 4444 4441
                       ...
                       код CVC - 789
    2. Кликнуть кнопку "Продолжить".
    Ожидаемый результат - появление подсказки - "Поле обязательно для заполнения" -> запрос в банк не отправляется.  
  ...
  #### 2.11 Проверка покупки тура по дебетовой карте с валидным заполнением всех полей, "номер карты" не из списка
    Предусловие:
    1. Открыть приложение "http://localhost:8080"
    2. Кликнуть по кнопке "Купить"
    Шаги:
    1. Заполнить поля: карта 4444 4444 4444 4444
                       ...
                       код CVC - 789
    2. Кликнуть кнопку "Продолжить".
    Ожидаемый результат - запрос отправляется в банк, банк присылает ответ - "Ошибка! Банк отказал в проведении операции".
...
### 3. Проверка покупки тура в кредит по данным дебетовой карты:
  ...
  #### 3.11 Проверка покупки тура в кредит с валидным заполнением всех полей, "номер карты" не из списка
    Предусловие:
    1. Открыть приложение "http://localhost:8080"
    2. Кликнуть по кнопке "Купить в кредит"
    Шаги:
  ...
  #### 3.12 Проверка покупки тура в кредит с валидным заполнением всех полей, "номер карты" со статусом DECLINED
    Предусловие:
    1. Открыть приложение "http://localhost:8080"
    2. Кликнуть по кнопке "Купить в кредит"
    Шаги:
...
### 4. Проверка корректности работы с базой данных и внесением данных:
  ...
  #### 4.5 Проверка поля "amount" таблицы 'payment_entry" при покупке тура по дебетовой карте с валидным заполнением всех полей                    статус карты DECLINED
    ...
    7. Получить количество записей в таблице "order_entry" и сравнить с суммой количества записей из таблиц "payment_entry"
       + "credit_request_entry". ОР - равенство количества записей
    8. Получить значение столбца "amount" из таблицы "payment_entry" последней по времени создания записи
    9. Сравнить полученное значение с "0"
    Ожидаемый результат - значение ячейки  = "0"        

  #### 4.6 Проверка поля "status" таблицы 'credit_request_entry" при покупке тура в кредит с валидным заполнением всех полей                    статус карты APPROVED
    Предусловие:
    1. Открыть приложение "http://localhost:8080"
    2. Кликнуть по кнопке "Купить в кредит"
    Шаги:
  ...
  #### 4.7 Проверка равенства "bank_id" из таблицы "credit_request_entry" с "credit_id" таблицы "order_entry" при покупке                      тура в кредит с валидным заполнением всех полей, статус карты APPROVED
    Предусловие:
    1. Открыть приложение "http://localhost:8080"
    2. Кликнуть по кнопке "Купить в кредит"
    Шаги:
  ...
  #### 4.8 Проверка поля "status" таблицы 'credit_request_entry" при покупке тура в кредит с валидным заполнением всех полей                    статус карты DECLINED
    Предусловие:
    1. Открыть приложение "http://localhost:8080"
    2. Кликнуть по кнопке "Купить в кредит"
    Шаги:
  ...
  #### 4.9 Проверка равенства "bank_id" из таблицы "credit_request_entry" с "credit_id" таблицы "order_entry" при покупке                      тура в кредит с валидным заполнением всех полей, статус карты DECLINED
    Предусловие:
    1. Открыть приложение "http://localhost:8080"
    2. Кликнуть по кнопке "Купить в кредит"
    Шаги:
