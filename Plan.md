...
### 4. Проверка корректности работы с базой данных и внесением данных:
  #### 4.1 Проверка поля "amount" таблицы 'payment_entry" при покупке тура по дебетовой карте с валидным заполнением всех полей
    Предусловие:
    1. Открыть приложение "http://localhost:8080"
    2. Кликнуть по кнопке "Купить"
    Шаги:
    ...
    6. Получить значение поля "amount" из таблицы "payment_entry" последней по времени создания записи
    7. Сравнить полученное значение с "45000"
    Ожидаемый результат - значение поля "amount" = "45000"

  #### 4.2 Проверка поля "status" таблицы 'payment_entry" при покупке тура по дебетовой карте с валидным заполнением всех полей
    Предусловие:
    1. Открыть приложение "http://localhost:8080"
    2. Кликнуть по кнопке "Купить"
    Шаги:
    ...
    5. Сделать запрос к БД - "SELECT COUNT (*) FROM payment_entry" для сравнения количества записей в БД с количеством
       из пункта 1. ОР - количество записей в БД увеличилось на 1.
    6. Получить значение поля "status" из таблицы "payment_entry" последней по времени создания записи
    7. Сравнить полученное значение с "APPROVED"
    Ожидаемый результат - значение поля "status" = "APPROVED"
